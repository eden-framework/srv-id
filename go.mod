module gitee.com/newtengroup/srv-id

go 1.14

replace k8s.io/client-go => k8s.io/client-go v0.18.8

require (
	github.com/eden-framework/apollo v0.0.1
	github.com/eden-framework/context v0.0.2
	github.com/eden-framework/courier v1.0.0
	github.com/eden-framework/eden-framework v1.1.1
	github.com/eden-framework/enumeration v1.0.0
	github.com/johnnyeven/libtools v0.0.0-20191126065708-61829c1adf46
	github.com/konsorten/go-windows-terminal-sequences v1.0.3 // indirect
	github.com/sirupsen/logrus v1.7.0
	github.com/spf13/cobra v0.0.5
)
